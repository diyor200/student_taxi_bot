...
from ..consts.consts import DRIVER_TYPE, DRIVER, CREATE_ROUTE, SEND_ROUTE_FORM, get_region_name_by_id, \
    ...
    DIRECTION_STATUS_TEXT, DIRECTION_STATUS_CANCEL, DIRECTION_STATUS_FULL
        ...
        case 'cancel_route' | 'full_route':
            try:
                if text_parts[0] == 'cancel_route':
                    status_code = DIRECTION_STATUS_CANCEL
                    status = DIRECTION_STATUS_TEXT[status_code]
                else:
                    status_code = DIRECTION_STATUS_FULL
                    status = DIRECTION_STATUS_TEXT[status_code]
                ...
                await db.update_direction_status(route_id=route['id'], status=status_code)
                await call.message.answer(text="✅Marshrut statusi muvaffaqiyatli o'zgartirildi")
                await call.bot.delete_message(message_id=call.message.message_id, chat_id=call.from_user.id)
            except Exception as ex:
                logging.error(ex)
                await call.message.answer(text="Xatolik yuzaga keldi, keyinroq qaytadan urinib ko'ring")
                return
        case 'change_user':
